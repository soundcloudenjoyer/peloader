...
ULONG_PTR lastpageadress = NULL;
SIZE_T lastpagesize = NULL;

VOID splitTextInPages(std::vector<Page>& v) {
	...
	SIZE_T totalSize = SC_Header[0].Misc.VirtualSize;
		...
		while ((start + PAGE_SIZE) < (textBase + totalSize)) {
			v.emplace_back(start, PAGE_SIZE);
			start += PAGE_SIZE;
		}

	v.emplace_back(start, (textBase + totalSize) - start);
...
LONG CALLBACK PageFaultHandler(PEXCEPTION_POINTERS ExceptionsInfo) {
	...
	if (faultAddr >= start && faultAddr < end) {
		...
		if (auto it = smallList.begin(); smallList.size() < 3) {
			smallList.emplace_back(faultAddr, PAGE_SIZE);
